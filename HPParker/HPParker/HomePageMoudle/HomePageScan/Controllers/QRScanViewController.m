...
//
//        UIBarButtonItem * rightBtn = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"ic_back.png"] style:UIBarButtonItemStyleDone target:self action:@selector(closedBtnAction:)];
...
- (void)initBaseUI
{
    ...
    btn.frame = CGRectMake(Width - 45, 27, 30, 30);
    ...
    _textLabel = [[UILabel alloc] initWithFrame:CGRectMake(50, (Height-pathWidth)/2-50 -30, pathWidth, 20)];
    _textLabel.text = @"对准二维码扫描";
    ...
    _inputBtn.frame = CGRectMake(40, orginY+60, 60, 60);
    _inputBtn.layer.cornerRadius = 30.0;
    _inputBtn.clipsToBounds = YES;

    _inputBtn.backgroundColor = [UIColor blueColor];

    [_inputBtn setTitle:@"手动" forState:UIControlStateNormal];
//    [_inputBtn setImage:[UIImage imageNamed:@"torch_n"] forState:UIControlStateNormal];
//    [_inputBtn setImage:[UIImage imageNamed:@"torch_s"] forState:UIControlStateSelected];
    [_inputBtn addTarget:self action:@selector(_inputBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_inputBtn];


    _inputBtnLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, orginY+130, 100, 30)];
    _inputBtnLabel.text = @"手动输入";
    _inputBtnLabel.textAlignment = NSTextAlignmentCenter;
    _inputBtnLabel.font = [UIFont systemFontOfSize:14];
    _inputBtnLabel.textColor = [UIColor whiteColor];
    [self.view addSubview:_inputBtnLabel];


    _torchBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _torchBtn.frame = CGRectMake(Width - 100, orginY+60, 60, 60);
    _torchBtn.backgroundColor = [UIColor blueColor];

    _torchBtn.layer.cornerRadius = 30.0;
    _torchBtn.clipsToBounds = YES;

    [_torchBtn setTitle:@"手电" forState:UIControlStateNormal];
...
//    [_torchBtn setImage:[UIImage imageNamed:@"torch_s"] forState:UIControlStateSelected];
    ...
    _tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(Width- 120, orginY+130, 100, 30)];
    _tipLabel.text = @"轻触照亮";
...
-(void)_inputBtnClick:(UIButton *)sender
{
